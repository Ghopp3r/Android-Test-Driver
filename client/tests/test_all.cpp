...
// Owner-scope lets two fds carry independent (pid, addr) trackers, so a "third fd installs on same addr" test
// can't tell "cleanup reaped alt's tracker" from "cleanup didn't and the new tracker sits alongside".
// Decidable form: exhaust every HW execute-BP slot from alt, close alt, then observe the primary can install
// one more from the same pid. A slot leak would surface as ENOSPC/EIO on the primary side.
...
// Two clients on the same (pid, addr): both installs succeed, alt's clearAll succeeds and empties only alt's
// trackers, primary's tracker survives, and primary's remove() still works.
...
// Install a W watchpoint on g_probe_word, trigger the store, poll getHits for the first hit, then confirm the
// tracker is one-shot: a follow-up store must not record another hit until re-install. First assertion that a
// non-execute callback actually ran.
...
// An older client compiled against the 280-byte hit layout would hit this command number; the new kernel refuses
// it with a stable errno so the caller sees the ABI break instead of silently misreading payloads.
...
// Old clients compiled against 32-byte caps must not have GET_CAPS overflow their buffer.
// We pass a 32-byte buffer with a canary that must be preserved after the ioctl.
...
// HWBP_GET_HITS used to collide with PTE_HOOK_INSTALL at 0x48 and the router dispatched PTE traffic into HWBP.
// The regression: PTE install must reach its own handler. We send an install req with a nonsense addr — the
// PTE handler rejects it with EINVAL/EFAULT (its own validator), not ENOTTY (unrecognised cmd) or EPROTO
// (legacy-hits code). Either of those would prove routing is broken.
